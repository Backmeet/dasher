...
try:
    ...
    argv = ["dasher.py", r"E:\vs code\files\Dasher\code\pong\code.dasher", r"E:\vs code\files\Dasher\code\pong\bin.txt"]
    # === Encoding Mapping ===
    encodeing = {
    ...
    def estimate_bin_length(cmd, args):
        ...
        if cmd == "var":
            ...
            return 2
        elif cmd in ("forever", "if", "while", "jump"):
            return 2
        elif cmd == "log":
            return 2
        elif cmd == "def":
        ...
        match cmd:
            ...
            case "var":
                ...
                match args[0]:
                    ...
                    case "math":  # var, math, ... (single-var or two-var)
                        ...
                        elif args[2].strip() in [op for op in opreationToBin if op not in [">>>", "<<<", "++", "--"]] and Exists(args[1]) and Exists(args[3]) and Exists(args[4]):
                            appended.extend([
                                f"{allocated[args[1]]} 0001",
                                "0000000000000000 0011",
                                f"{allocated[args[3]]} 0001",
            ...
            case "io": # io, set | get
                ...
                match args[0]:
                    ...
                    case "set":  # io, set, var, port (1-4)
                        if Exists(args[1]) and int(args[2]) in [1, 2, 3, 4]:
                            appended.extend([
                                f"{allocated[args[1]]} 0001",
                                f"000000000000{format(ioTable[int(args[2])], '04b')} 0011\n"
                            ])

                    case "get":  # io, get, var, port (1-4)
                        if Exists(args[1]) and int(args[2]) in [1, 2, 3, 4]:
                            appended.extend([
                                f"{allocated[args[1]]} 0001",
                                f"000000000000{format(ioTable[int(args[2])], '04b')} 0100\n"
                            ])
            ...
            case "def": # def, name
                funstionNamesToLines.update({args[0]:i})

            case "call": # call, name
                if args[0] in funstionNamesToLines.keys():
                    functionline = funstionNamesToLines[args[0]]
                    binline = LineToBin[functionline]
                    x = format(binline, '016b')
                    appended.extend([
                        f"{x} 1100\n"
                    ])
    ...
    program.close()
    outF.close()
    input("Ended compiling:")
except Exception as e:
    print(f"{traceback.extract_tb(e.__traceback__)}")
    print(f"Exception {e}")
    print(f'''
Compier Dump:
Current Line: {i}
...
Line to bin: {LineToBin}
Function to Line: {funstionNamesToLines}
alocated: {allocated}
''')
    input("Ended compiling:")